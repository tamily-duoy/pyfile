...
def import_data_format_iris(file):
    ...
    with open(str(file), 'r') as f:
        for line in f:
            ...
            current_dummy = []
            # for j in range(0, len(current) - 1):  #原码
            for j in range(0, len(current) - 2):  #修正原码
...
def fuzzy(data, cluster_number, m):
    ...
    while (True):
        ...
        # 计算聚类中心
        C = []
        for j in range(0, cluster_number-1):
...
if __name__ == '__main__':
    # 加载数据
    data, cluster_location = import_data_format_iris("test.txt")
    # print_matrix(data)
